...
_NAMED_SECRET = re.compile(
    r"(?i)(\b(?:[a-z][a-z0-9]*[_-])*(?:api[_-]?key|access[_-]?token|auth(?:orization)?|password|passwd|"
    r"secret|token|client[_-]?secret)\b[\"']?\s*[:=]\s*)"
    r'''("[^"\n]*"|'[^'\n]*'|[^\s,;&"']+)'''
)
