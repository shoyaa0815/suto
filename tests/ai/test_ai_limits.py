...
async def test_developer_job_interrupts_work_at_elapsed_time_limit(
    monkeypatch,
    tmp_path,
):
    async def fake_chat(session, messages, tool_schemas, think=False):
...
async def test_developer_job_blocks_repeated_identical_tool_calls(
    monkeypatch,
    tmp_path,
):
...
async def test_developer_job_blocks_tool_calls_over_budget(monkeypatch, tmp_path):
